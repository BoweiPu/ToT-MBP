...
from utils.model_utils import update_teacher_params
...
from utils.wrapper import ImgFeatDataset
...
def extend_cfg(cfg):
    ...
    cfg.MODEL.BACKBONE.PRETRAINED="openai"
    cfg.SEED = 7777
...
def setup_prompt_training_utils(args, model):
    ...
    for key, value in model.named_parameters():
        if 'text_adapter' in key or\
         'down_' in key or 'up_'in key  and 'text_adapter_1' not in key :
...
def train(args, model, tr_loader, val_loader):
    ...
    model.cal_test_img_feat(val_loader)
    best=model.eval_with_feat(-1)
    for i in range(50):
...
def main(args):
    ...
    setup_my_epochs(args, dataset_name)
    ...
    #test_loader=ReBuild_data_loader(cfg,model,test_loader)
    if args.zero_shot:
