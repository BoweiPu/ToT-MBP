...
class MLClassification(EvaluatorBase):
    ...
    def evaluate(self):
        ...
        if self.activate_func == 'default_merge_aux':
            mAP_score ,ap_def= mAP(targets, preds.cpu().numpy())
            print("mAP score default:", mAP_score)
            if len(self._y_pred_aux) > 0:
                mAP_score_aux ,ap_aux= mAP(targets, preds_aux.cpu().numpy())
                ...
                mAP_score_,_ = mAP(targets, preds_merge)
                print("mAP score merged : preds + preds_aux:", mAP_score_)
                mAP_score = mAP_score_
                #print(f'logit:{preds}')
                #print(f'logit_local:{preds_aux}')
        elif self.activate_func == 'only_local':
            mAP_score,_ = mAP(targets, preds_aux.cpu().numpy())
