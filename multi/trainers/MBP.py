...
from .utils import mask_new, soft_cross_entropy, softmax_sigmoid_BCEloss, \
...
class DenseCLIP(nn.Module):
    ...
    def forward(self, image=None, captions=None, if_test=False,label=None,epoch=0):
        if if_test:        
            ...
            tmp_scale = self.cfg.TRAIN.spatial_SCALE_image 
            ...
            if self.cfg.MODEL.r>0.01:
                index_labels = torch.nonzero(label, as_tuple=False)
                mask_head=torch.zeros(image_feat.shape[0],image_feat.shape[2]).to(self.device)
                mask_local=torch.zeros(image_feat.shape[0],image_feat.shape[2]).to(self.device)
                count = torch.zeros(image_feat.shape[0],1).to(self.device)
                ...
                image_feat=image_feat*mask_local[:,None,:]
                #image_feature_=image_feature_*mask_head/count
 # BD    
            image_feature_ = image_feat[torch.arange(image_feat.shape[0]), captions.argmax(dim=-1)]  
            ...
            tmp_scale =self.cfg.TRAIN.spatial_SCALE_text
            prob_spatial = torch.nn.functional.softmax(logits_neg * tmp_scale, dim=0)
            logits_local = torch.sum( logit_scale*logits_neg * prob_spatial, dim=0)
...
class MBP(TrainerX):
    ...
    def get_init_head_weight(self,):
        ...
        head_h=torch.quantile(head_filter, 0.1)
    ...
    def forward_backward(self, batch):
        ...
        if prec == "amp":
            ...
            output, output_local, _, _ = self.model(None, image,label=label,epoch=self.epoch)
