...
def Smask(feas,rate):
        ...
        feas=feas-feas.mean(dim=0)
        condition =  (feas > rate * feas.std()) | (feas < -rate * feas.std())
        mask[condition]=1
        return mask
def top_mask(tensor,num):
        
        mask = torch.zeros_like(tensor, dtype=torch.uint8)  # 与tensor同形状的全1 mask

        # 获取最后一维中最大的n个值的位置（indices）
        _, max_indices = torch.topk(tensor, num, dim=-1, largest=True)
        # 获取最后一维中最小的n个值的位置（indices）
        _, min_indices = torch.topk(tensor, num, dim=-1, largest=False)

        # 为了在这些位置上将mask设置为0，我们可以遍历每一个位置，并将对应的mask值设为0

        # 遍历最大值索引，将mask对应位置设置为0
        for i in range(max_indices.size(-1)):
            mask.scatter_(-1, max_indices.select(-1, i).unsqueeze(-1), 1)

        # 同理，遍历最小值索引，将mask对应位置设置为0
        for i in range(min_indices.size(-1)):
            mask.scatter_(-1, min_indices.select(-1, i).unsqueeze(-1), 1)
        return mask*tensor

def mask_new(feas,rate):
    mask = torch.zeros_like(feas)
    feas=feas-feas.mean()
