...
from trainers.groupBlock import Net, QueryBlock
...
class DenseCLIP(nn.Module):
    ...
    def forward(self, image=None, captions=None, if_test=False, mean=None):
            image_features= self.text_encoder(captions, None, if_embedding=False, if_sequence=True)
           ...
           #==============norm======================
            ...
            noise=self.cfg.MODEL.noisy_rate* torch.randn(image_feature_.shape).to(self.device)
            image_feature_ =(self.img_feature_mean- self.text_feature_mean).to(self.device)*noise+image_feature_

            noise= self.cfg.MODEL.noisy_rate*torch.randn(image_features.shape).to(self.device)
            image_features =(self.img_feature_mean- self.text_feature_mean).to(self.device)*noise+image_features
            ...
            local_attn=self.QueryBlock(x=image_features,mask=text_mask) #B,L,N
    ...
    def eval_mutiscale_img(self,img,split_2,split_3):
        img_feature=self.model.visual(img)

        img_feature_2_=[]
        for split in torch.unbind(split_2, dim=1):
            img_feature_2_.append(self.model.visual(split))
        img_feature_2=torch.stack(img_feature_2_,dim=1)

        img_feature_3_=[]
        for split in torch.unbind(split_3, dim=1):
            img_feature_3_.append(self.model.visual(split))
        img_feature_3=torch.stack(img_feature_3_,dim=1)
        
        local_attn_2=self.QueryBlock(x=img_feature_2,mask=None) #B,L,N
        logits_local_2=self.local_head(img_feature_2)*local_attn_2

        local_attn_3=self.QueryBlock(x=img_feature_3,mask=None) #B,L,N
        logits_local_3=self.local_head(img_feature_3)*local_attn_3


        logits_head=self.head(self.QueryBlock.scale2 *img_feature) #B,N

        logits_local=torch.sum(logits_local_3 , dim=1)*0.5+torch.sum(logits_local_2 , dim=1)*0.5

        return logits_head,logits_local,None,None
    
    def train_feature(self, feature, captions):
        ...
        text_feature_ =(self.img_feature_mean- self.text_feature_mean).to(self.device)*noise+text_feature_

        noise= (self.cfg.MODEL.noisy_rate*torch.randn(feature.shape)+1).to(self.device)
        feature =(self.img_feat_mean- self.text_feat_mean).to(self.device)*noise+feature

        #================global_logit=================

        logits_head=self.head(self.QueryBlock.scale1 *text_feature_) #B,N

        #================local_logit=================
        text_mask = (captions == 0).long() * (-10000)  # BL


        local_attn=self.QueryBlock(x=feature,mask=None) #B,L,N

        logits_local=self.local_head(self.QueryBlock.scale2 *feature)*local_attn

        logits_local=torch.sum(logits_local, dim=1)


        return logits_head*0.5 +logits_local*0.5,None , None,None

    def eval_feature_split(self,img_feature,img_feature_2,img_feature_3):
        
        img_feature = img_feature / img_feature.norm(dim=-1, keepdim=True)
        img_feature_2 = img_feature_2 / img_feature_2.norm(dim=-1, keepdim=True)
        img_feature_3 = img_feature_3 / img_feature_3.norm(dim=-1, keepdim=True)
        ...
        logits_local=torch.sum(logits_local_3 , dim=1)+torch.sum(logits_local_2 , dim=1)

        return logits_head,logits_local,None,None
    def eval_feature_net(self,img_feature,img_feat):
        img_feature_2,img_feature_3=self.net.interface(img_feat)
        img_feature_2=torch.stack(img_feature_2,dim=1).squeeze()
        img_feature_3=torch.stack(img_feature_3,dim=1).squeeze()
    ...
    def eval_ori(self,feat,feature):
            
         
            image_feature_= feature / feature.norm(dim=-1, keepdim=True)
            image_features = feat / feat.norm(dim=-1, keepdim=True)

            logits_head=self.head(self.QueryBlock.scale1 *image_feature_) #B,N

            local_attn=self.QueryBlock(x=image_features,mask=None) #B,L,N

            logits_local=self.local_head(self.QueryBlock.scale2 *image_features)*local_attn

            logits_local=torch.sum(logits_local , dim=1)

            return logits_head*0.5 +logits_local*0.5,None , None,None
    
    def eval_low(self,feat,feature):
            ...
            logits_local=self.QueryBlock.low_dim(image_features,image_feature_) #B,L,N

            return logits_head,logits_local,None,None
    
   
    def train_feature_low(self, feature, captions):
        ...
        logits_local=self.QueryBlock.low_dim(feat=feature,features=text_feature_,mask=text_mask) #B,L,N
