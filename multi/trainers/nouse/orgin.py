...
from trainers.model.groupBlock import GroupingBlock, QueryBlock
...
class DenseCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model, return_interm_layers=False):
        ...
        dim=512 if self.if_vit else 1024
 ...
 #image_feat = self.text_encoder(captions, None, if_embedding=False, if_sequence=True,gauss=gaussian_tokens_expanded)
            ...
            add_noise = 0.1 * torch.randn(image_feature_.shape).to(self.device)
            ...
            image_feature_ = image_feature_ / image_feature_.norm(dim=-1, keepdim=True)
            image_feature_ = (image_feature_ + add_noise)
            image_features = image_features / image_features.norm(dim=-1, keepdim=True)
            text_features = text_features / text_features.norm(dim=-1, keepdim=True)
            text_features_neg = text_features_neg / text_features_neg.norm(dim=-1, keepdim=True)
...
class Caption_distill_double(TrainerX):
    def model_inference(self, input, mean=None):
        return self.model(input, if_test=True, mean=mean)
    ...
    def build_model(self):
        ...
        self.text_mean_feature = None
        self.image_mean_feature = None
    ...
    def forward_backward(self, batch):
        ...
        if prec == "amp":
            ...
            output, output_local, image_feature_, image_feat = self.model(None, image)
