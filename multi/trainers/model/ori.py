...
class DenseCLIP(nn.Module):
    ...
    def forward(self, image=None, captions=None, if_test=False,label=None,feat=None):
            if not if_test:
                image_features= feat
                ...
                image_feature_=image_feature_+(0.1*self.noise*noise).to(self.device)
                noise=torch.randn(image_features.shape)
                image_features=image_features+(0.1*self.noise*noise).to(self.device)
